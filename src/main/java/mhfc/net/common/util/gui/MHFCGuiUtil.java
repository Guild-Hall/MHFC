...
public class MHFCGuiUtil {
	...
	public static void drawTexturedBoxFromBorder(int x, int y, int width,
			int height) {
		drawTexturedBoxFromBorder(x, y, width, height,
				Math.min(Math.min(15, width / 2), height / 2));
	}

	public static void drawTexturedBoxFromBorder(int x, int y, int width,
			int height, int borderSize) {
		drawTexturedBoxFromBorder(x, y, width, height, borderSize,
				borderSize / 256f);
	}

	public static void drawTexturedBoxFromBorder(int x, int y, int width,
			int height, int borderSize, float borderUV) {
		drawTexturedBoxFromBorder(x, y, width, height, borderSize, borderUV, 1,
				1);
	}

	public static void drawTexturedBoxFromBorder(int x, int y, int width,
			int height, int borderSize, float borderUV, float maxU, float maxV) {
		drawTexturedBoxFromBorder(x, y, width, height, borderSize, borderUV,
				borderUV, maxU, maxV);
	}

	public static void drawTexturedBoxFromBorder(int x, int y, int width,
			int height, int borderSize, float borderU, float borderV,
