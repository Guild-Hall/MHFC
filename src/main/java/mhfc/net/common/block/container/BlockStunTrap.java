...
public class BlockStunTrap extends BlockContainer {
	...
	public void randomDisplayTick(World world, int x, int y, int z,
		...
		world.spawnParticle("largesmoke", f1 + f4, f2, (double) f3 + f5, 0D,
				0D, 0D);
	}
	...
	public void onNeighborBlockChange(World par1World, int par2, int par3,
		...
		if (!World
				.doesBlockHaveSolidTopSurface(par1World, par2, par3 - 1, par4)
				&& !BlockFence.func_149825_a(par1World.getBlock(par2, par3 - 1,
						par4))) {
			flag = true;
		}
	...
	public void onEntityCollidedWithBlock(World world, int i, int j, int k,
		...
		entityliving.addPotionEffect(new PotionEffect(
				MHFCPotionRegistry.stun.id, 500, 10));
		return;
