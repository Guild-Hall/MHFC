...
import mhfc.net.common.entity.monster.*;
...
public class MHFCEntityRegistry {
	...
	static {
		...
		greatjaggiID = getMobID(EntityGreatJaggi.class,
			MHFCReference.mob_greatjaggi_name, 0xff119f91, 0xff929ff);
		deviljhoID = getMobID(EntityDeviljho.class,
			MHFCReference.mob_deviljho_name, 0x6ff81ff, 0xff11d830);
		nargacugaID = getMobID(EntityNargacuga.class,
			MHFCReference.mob_nargacuga_name, 0xf351631, 0x516f13f);
		barrothID = getMobID(EntityBarroth.class,
			MHFCReference.mob_barroth_name, 0x6ffffff, 0x654321);
	...
	private static int getProjectileID(Class<? extends Entity> clazz,
		...
		EntityRegistry.registerModEntity(clazz, name, projectileID, mod, 64, 10,
			true);
	...
	private static boolean isRegistered(Class<?> clazz) {
		return registeredMobs.contains(clazz) || registeredProjectiles.contains(
			clazz);
	}
