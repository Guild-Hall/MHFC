...
public class ModLivingEvent {
	...
	public static void onEntityUpdate(LivingUpdateEvent event) {
		...
		final PotionEffect activeEffect = event.getEntityLiving()
	...
	public static void onDamageDealtToArmor(LivingHurtEvent event) {
		final ArmorBase armor = ArmorBase.instance;
		final DamageSource damageSrc = event.getSource();
		final double damage = event.getAmount();
		...
		if (ent != null && ent instanceof EntityPlayer) {
			if (damageSrc.getImmediateSource() instanceof CreatureAttributes)
				if (ent.getArmorInventoryList() != null && ent.getArmorInventoryList() instanceof ArmorBase) {
					/**
					 * Refines the damage analyzer based from Monster Hunter Series but in a special case which the
					 * default health is set to 20 for players.
					 **/
					final float newDamage = (float) Math
							.abs(((damage * 80) / (armor.getInitialDefenseValue() + 80)) * 0.20);
	...
	public static void onEntitySpawn(LivingSpawnEvent event) {
		if (event.getWorld().provider instanceof WorldProviderQuesting) {
