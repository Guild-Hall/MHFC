...
import mhfc.net.common.entity.projectile.EntityProjectile;
import mhfc.net.common.util.Attributes;
import mhfc.net.common.weapon.range.ReloadHelper;
...
public abstract class ComponentRange extends AbstractWeaponClass {
	...
	public ComponentRange(RangedSpecs rangedspecs) {
	...
	public void addItemAttributeModifiers(
		...
		multimap.put(
				Attributes.RELOAD_TIME.getAttributeUnlocalizedName(),
				new AttributeModifier(weapon.getUUID(),
						"Weapon reloadtime modifier", rangedSpecs
								.getReloadTime(), 0));
	}
	...
	public void soundCharge(ItemStack itemstack, World world,
			EntityPlayer entityplayer) {}

	public final void postShootingEffects(ItemStack itemstack,
			EntityPlayer entityplayer, World world) {
		effectPlayer(itemstack, entityplayer, world);
		effectShoot(world, entityplayer.posX, entityplayer.posY,
				entityplayer.posZ, entityplayer.rotationYaw,
				entityplayer.rotationPitch);
	}

	public abstract void effectReloadDone(ItemStack itemstack, World world,
			EntityPlayer entityplayer);

	public abstract void fire(ItemStack itemstack, World world,
			EntityPlayer entityplayer, int i);

	public abstract void effectPlayer(ItemStack itemstack,
			EntityPlayer entityplayer, World world);

	public abstract void effectShoot(World world, double x, double y, double z,
	...
	public boolean hasAmmoAndConsume(ItemStack itemstack, World world,
			EntityPlayer entityplayer) {
	...
	public boolean hasAmmo(ItemStack itemstack, World world,
			EntityPlayer entityplayer) {
		return entityplayer.capabilities.isCreativeMode
				|| EnchantmentHelper.getEnchantmentLevel(
						Enchantment.infinity.effectId, itemstack) > 0
	...
	public static enum RangedSpecs {
		BOWGUN("mhfc:bullet", "bowgunheavy", 250, 12);
		...
		public int getReloadTime() {
			if (reloadTime < 0 && MHFCMain.instance != null) {
				reloadTime = MHFCMain.config.getReloadTime(reloadTimeTag);
				MHFCMain.logger.debug("Found reaload time " + reloadTime
						+ " for " + reloadTimeTag + " @" + this);
			}
			return reloadTime;
		}
		...
		public Item getAmmoItem() {
			if (ammoItem == null && ammoItemTag != null) {
				...
				MHFCMain.logger.debug("Found item " + ammoItem + " for "
						+ ammoItemTag + " @" + this);
				ammoItemTag = null;
			}
			return ammoItem;
