...
/*
 * For future basing of every Weapon Class of MHFC . Will remove extending the ItemSword soon to make it more stable.
...
public abstract class WeaponMelee extends ItemSword implements IWeaponCluster {
	...
	public void addInformation(
		...
		if (poisontype) {
			des2 = MHFCWeaponClassingHelper.poisonname;
		}
		if (firetype) {
			des2 = MHFCWeaponClassingHelper.firename;
		}
		if (icetype) {
			des2 = MHFCWeaponClassingHelper.icename;
		}
		if (watertype) {
			des2 = MHFCWeaponClassingHelper.watername;
		}
		if (dragontype) {
			des2 = MHFCWeaponClassingHelper.dragonname;
		}
		if (thundertype) {
			des2 = MHFCWeaponClassingHelper.thundername;
		}
		if (blighttype) {
			des2 = MHFCWeaponClassingHelper.blightname;
		}
		if (windtype) {
			des2 = MHFCWeaponClassingHelper.windname;
		}
	}
	...
	public boolean onBlockDestroyed(
		...
		return meleecomp.onBlockDestroyed(itemstack, world, block, j, k, l, entityliving);
	}
	...
	public Multimap<String, AttributeModifier> getAttributeModifiers(ItemStack stack) {
		...
		meleecomp.addItemAttributeModifiers(multimap, stack);
		return multimap;
	}
