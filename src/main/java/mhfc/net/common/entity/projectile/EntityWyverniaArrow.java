...
public class EntityWyverniaArrow extends EntityArrow implements IProjectile {
	...
	public EntityWyverniaArrow(World world) {
		super(world);
		setSize(Utilities.mhfc_vanilla_size_x, Utilities.mhfc_vanilla_size_y);
	}
	...
	public EntityWyverniaArrow(
			World world,
			EntityLivingBase theShooter,
		...
		posY = theShooter.posY + theShooter.getEyeHeight() - 0.10000000149011612D;
		double d0 = target.posX - theShooter.posX;
		double d1 = target.getCollisionBoundingBox().minY + target.height / 3.0F - posY;
		...
		if (d3 >= 1.0E-7D) {
			...
			setThrowableHeading(d0, d1 + f4, d2, unknownA, unknownB);
		}
	}
	...
	public EntityWyverniaArrow(World world, EntityLivingBase theShooter, float unknownA) {
		...
		setLocationAndAngles(
				theShooter.posX,
				theShooter.posY + theShooter.getEyeHeight(),
				theShooter.posZ,
				theShooter.rotationYaw,
				theShooter.rotationPitch);
	...
	public void setThrowableHeading(double par1, double par2, double par3, float par4, float par5) {
		...
		par2 /= x2;
		par3 /= x2;
		par1 += rand.nextGaussian() * (rand.nextBoolean() ? -1 : 1) * 0.007499999832361937D * par5;
		par2 += rand.nextGaussian() * (rand.nextBoolean() ? -1 : 1) * 0.007499999832361937D * par5;
		par3 += rand.nextGaussian() * (rand.nextBoolean() ? -1 : 1) * 0.007499999832361937D * par5;
		par1 *= par4;
		par2 *= par4;
		par3 *= par4;
		motionX = par1;
		...
		prevRotationPitch = rotationPitch = (float) (Math.atan2(par2, x3) * 180.0D / Math.PI);
		ticksisInGround = 0;
	}
	...
	public void setVelocity(double dizX, double disY, double disZ) {
		...
		if (prevRotationPitch == 0.0F && prevRotationYaw == 0.0F) {
			...
			prevRotationPitch = rotationPitch = (float) (Math.atan2(disY, y1) * 180.0D / Math.PI);
	...
	public void onUpdate() {
		...
		if (blockz.getMaterial() != Material.AIR) {
			...
			AxisAlignedBB axisalignedbb = blockz
					.getCollisionBoundingBoxFromPool(worldObj, arrowHeadingX, arrowHeadingY, arrowHeadingZ);
							...
							if (knockbackStrength > 0) {
								...
								if (f4 > 0.0F) {
									movingobjectposition.entityHit.addVelocity(
											motionX * knockbackStrength * 0.6000000238418579D / f4,
											0.1D,
