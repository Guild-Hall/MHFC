...
public class Rathalos extends CreatureAttributes<Rathalos>
	...
	public static enum Stances implements IStancedEntity.IStance<Rathalos> {
		GROUND, FLYING, FALLING, BLINDED {
			@Override
			public void onAttackStart(
					IExecutableAction<? super Rathalos> attack,
					Rathalos entity) {
				entity.confusedAttacks++;
			}

			@Override
			public void onAttackEnd(IExecutableAction<? super Rathalos> attack,
					Rathalos entity) {
		...
		@Override
		public void onAttackStart(IExecutableAction<? super Rathalos> attack,
				Rathalos entity) {
		}
	...
	protected IActionManager<Rathalos> constructActionManager() {
		...
		attackManager.registerAction(new AIBreathe(this,
				"mhfc:models/Rathalos/rathalosidle.mcanm", 65, 5));
		attackManager.registerAction(new AIWander<Rathalos>(this,
				"mhfc:models/Rathalos/RathalosWalk.mcanm", 125, 1F, 0.07F, 3F,
				0, 120, 1, 15));
		attackManager.registerAction(new AIAngleWhip<Rathalos>(
				"mhfc:models/Rathalos/rathalostailswiperight.mcanm", 35, 12,
				50.5F, 5, MHFCSoundRegistry.getRegistry().rathalosTailWhip, 9,
				6, 3, 180, 10));
		attackManager.registerAction(new AIBite(this,
				"mhfc:models/Rathalos/RathalosBiteLeft.mcanm", 50, 35, 95F, 5F,
				MHFCSoundRegistry.getRegistry().rathalosBite, 10, true, 30,
				40));
		attackManager.registerAction(new AIBite(this,
				"mhfc:models/Rathalos/RathalosBiteRight.mcanm", 50, 35, 95F, 6F,
				MHFCSoundRegistry.getRegistry().rathalosBite, 10, true, 30,
				40));
		...
		attackManager.registerAction(setDeathAction(
				new AIDeath(this, "mhfc:models/Rathalos/RathalosDeath.mcanm",
	...
	protected void dropFewItems(boolean par1, int par2) {
		...
		for (var4 = 0; var4 < 13; ++var4) {
			dropItemRand(
					SubTypedItem.fromSubItem(MaterialSubType.RATHALOSSHELL, 1));
		}
		for (var4 = 0; var4 < 8; ++var4) {
			dropItemRand(SubTypedItem
					.fromSubItem(MaterialSubType.RATHALOSWEBBING, 1));
			dropItemRand(SubTypedItem
					.fromSubItem(MaterialSubType.RATHALOSMARROW, 1));

		}
		for (var4 = 0; var4 < 1; ++var4) {
			dropItemRand(
					SubTypedItem.fromSubItem(MaterialSubType.RATHALOSWING, 1));

		}
		dropItemRand(
				SubTypedItem.fromSubItem(MaterialSubType.RATHALOSPLATE, 1));
	}
}
