...
public class WeaponHHMetalBagpipe extends SiegeClass {
	...
	public boolean hitEntity(ItemStack stack, EntityLivingBase ent, EntityLivingBase player){
		...
		{
			((EntityLivingBase) entity).addPotionEffect(new PotionEffect(Potion.resistance.id, 100, 1));
		}
		if(rand.nextInt(40) == 0){
			((EntityLivingBase) entity).addPotionEffect(new PotionEffect(Potion.jump.id, 60, 3));
		}
		float damage = 0.0f;
		...
		if(Cooldown.canUse(iStack, COOLDOWN)){
				...
				for (int i = 0; i < list.size(); i++) {
					EntityLivingBase entity = (EntityLivingBase)list.get(i);
				    	entity.addPotionEffect(new PotionEffect(Potion.regeneration.id, 500, 1));
				    }
