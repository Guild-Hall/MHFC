...
public class GoalReference {
	...
	public static class GoalRefSerializer implements JsonDeserializer<GoalReference> {
	...
	public static GoalReference constructFromJson(JsonElement element, JsonDeserializationContext context) {
		...
		if (element.isJsonPrimitive() && element.getAsJsonPrimitive().isString()) {
			return new GoalReference(JsonUtils.getJsonElementStringValue(element, "Goal Reference"));
		} else if (element.isJsonObject()) {
			GoalDescription desc = context.<GoalDescription>deserialize(element, GoalDescription.class);
