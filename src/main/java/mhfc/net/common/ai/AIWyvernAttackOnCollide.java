...
public class AIWyvernAttackOnCollide extends EntityAIBase {
	...
	public AIWyvernAttackOnCollide(EntityCreature par1EntityLiving,
			Class<?> par2Class, float par3, boolean par4) {
		this(par1EntityLiving, par3, par4);
		classTarget = par2Class;
	}

	public AIWyvernAttackOnCollide(EntityCreature par1EntityLiving, float par2,
			boolean par3) {
		attackTick = 0;
		maxAttackTick = 20;
	...
	public boolean continueExecuting() {
		EntityLivingBase var1 = attacker.getAttackTarget();
		return var1 != null ? var1.isEntityAlive() ? field_75437_f ? attacker
				.isWithinHomeDistance(MathHelper.floor_double(var1.posX),
						MathHelper.floor_double(var1.posY),
						MathHelper.floor_double(var1.posZ)) : !attacker
				.getNavigator().noPath() : false : false;
	}
	...
	public void updateTask() {
		attacker.getLookHelper().setLookPositionWithEntity(entityTarget, 30F,
				30F);
		if ((field_75437_f || attacker.getEntitySenses().canSee(entityTarget))
				&& --field_75445_i <= 0) {
			field_75445_i = 4 + attacker.getRNG().nextInt(7);
			attacker.getNavigator().tryMoveToEntityLiving(entityTarget,
					moveSpeed);
		}
		attackTick = Math.max(attackTick - 1, 0);
		double var1 = attacker.width * 2.0F * attacker.width * 2.0F;
		if (attacker.getDistanceSq(entityTarget.posX,
				entityTarget.boundingBox.minY, entityTarget.posZ) <= var1
				&& attackTick <= 0) {
