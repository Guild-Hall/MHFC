...
@Mod(modid = ResourceInterface.main_modid, updateJSON = "https://raw.githubusercontent.com/Guild-Hall/MHFC/updates/update.json")
public class MHFCMain {
	private static IPhaseAccess<FMLConstructionEvent, Void> constructedPhaseAccess = Services.instance
			.<FMLConstructionEvent, Void>registerPhase("mod constructed")
			.setDefaultShutdownContext(null);
	...
	private static IPhaseAccess<FMLPreInitializationEvent, Void> preInitPhaseAccess = Services.instance
			.<FMLPreInitializationEvent, Void>registerPhase(
					"mod pre initialized")
			.setDefaultShutdownContext(null)
	...
	private static IPhaseAccess<FMLInitializationEvent, Void> initPhaseAccess = Services.instance
			.<FMLInitializationEvent, Void>registerPhase("mod initialized")
			.setDefaultShutdownContext(null).declareRequirement(preInitPhase);
	...
	private static IPhaseAccess<FMLPostInitializationEvent, Void> postInitPhaseAccess = Services.instance
			.<FMLPostInitializationEvent, Void>registerPhase(
					"mod post initialized")
			.setDefaultShutdownContext(null).declareRequirement(initPhase);
	...
	private static IPhaseAccess<FMLServerStartingEvent, FMLServerStoppedEvent> serverRunningPhaseAccess = Services.instance
			.<FMLServerStartingEvent, FMLServerStoppedEvent>registerPhase(
					"server running")
	...
	private static IPhaseAccess<FMLServerStartedEvent, FMLServerStoppingEvent> serverActivePhaseAccess = Services.instance
			.<FMLServerStartedEvent, FMLServerStoppingEvent>registerPhase(
					"server active")
	...
	public final static CreativeTabs mhfctabs = new MHFCTab(
			CreativeTabs.getNextID(), "mhfc");
	private final static Logger logger = LogManager
			.getLogger(ResourceInterface.main_modid);
	...
	protected void onCreation(FMLConstructionEvent event) {
		staticInit();
		this.mhfcContainer = Loader.instance().getIndexedModList()
				.get(ResourceInterface.main_modid);
		if (mhfcContainer.getMod() != this) {
			Message errorMessage = logger().getMessageFactory().newMessage(
					"Mod container found for 'mhfc' is {}, not {}",
					mhfcContainer.getMod(), this);
			logger.fatal(errorMessage, new IllegalStateException(
	...
	protected void onPreInit(FMLPreInitializationEvent event) {
		...
		UpdateSystem.init();
		MHFCMain.logger().info("Starting MHFC ( "
				+ ResourceInterface.getMetadata().version + " ) ");
		...
		ValueHelper.setPrivateFinalValue(RangedAttribute.class,
				(RangedAttribute) SharedMonsterAttributes.MAX_HEALTH, 500000,
	...
	protected void onServerStarting(FMLServerStartingEvent event) {
		...
		event.registerServerCommand(
				new CommandPortableSchematic(WorldEdit.getInstance()));
	...
	protected void onWorldLoad(WorldEvent.Load event) {
		// Load a 3x3 around spawn to make sure that it populates and calls our
		// hooks.
		if (!event.getWorld().isRemote
				&& event.getWorld() instanceof WorldServer) {
			WorldServer world = (WorldServer) event.getWorld();
			int spawnX = (int) (world.getWorldInfo().getSpawnX()
					/ world.provider.getMovementFactor() / 16);
			int spawnZ = (int) (world.getWorldInfo().getSpawnZ()
					/ world.provider.getMovementFactor() / 16);
			for (int x = -1; x <= 1; x++) {
				for (int z = -1; z <= 1; z++) {
