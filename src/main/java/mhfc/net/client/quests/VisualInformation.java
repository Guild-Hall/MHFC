...
public class VisualInformation implements IRunningInformation {
	...
	public VisualInformation(VisualDefinition definition) {
	...
	@Override
	public CompositeString getMemberInformation() {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	public CompositeString getShortStatus() {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	public CompositeString getLongStatus() {
		// TODO Auto-generated method stub
		return null;
	}
	...
	public void drawInformation(
			...
			int renderFrame) {
		int FPS = 120;
		drawInformation(positionX, positionY, width, height, Math.max(((renderFrame / FPS) % 4) - 1, 0), fontRenderer);
	}
	...
	protected void drawClientDescription(int positionX, int currentY, int width, FontRenderer fontRenderer) {
		...
		String TAG_CLIENT = StatCollector.translateToLocal(MHFCReference.unlocalized_tag_client), //
				TAG_DESCRIPTION = StatCollector.translateToLocal(MHFCReference.unlocalized_tag_description);
		...
		currentY += MHFCGuiUtil
				.drawTextAndReturnHeight(fontRenderer, TAG_DESCRIPTION, positionX + BORDER, currentY, 0, COLOUR_HEADER);
		currentY += LINE_SEPERATION;
	...
	protected void drawAimsFails(
			...
			int currentY,
			FontRenderer fontRenderer) {
		String TAG_AIMS = StatCollector.translateToLocal(MHFCReference.unlocalized_tag_aims), //
				TAG_FAILS = StatCollector.translateToLocal(MHFCReference.unlocalized_tag_fails);
		currentY += MHFCGuiUtil
				.drawTextAndReturnHeight(fontRenderer, TAG_AIMS, positionX + BORDER, currentY, 0, COLOUR_HEADER);
		currentY += LINE_SEPERATION;
		currentY += MHFCGuiUtil.drawTextAndReturnHeight(
				fontRenderer,
				getAims(),
		...
		currentY = Math.max(currentY, positionY + height / 2);
		currentY += MHFCGuiUtil
				.drawTextAndReturnHeight(fontRenderer, TAG_FAILS, positionX + BORDER, currentY, 0, COLOUR_HEADER);
		currentY += LINE_SEPERATION;
		currentY += MHFCGuiUtil.drawTextAndReturnHeight(
				fontRenderer,
				getFails(),
