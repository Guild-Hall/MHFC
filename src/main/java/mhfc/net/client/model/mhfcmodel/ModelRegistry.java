...
public class ModelRegistry implements IResourceManagerReloadListener {
	...
	public static ModelMHMD loadFrom(ResourceLocation resLoc) {
		return instance.loadModelFrom(resLoc);
	}

	private HashMap<ResourceLocation, ModelMHMD> cachedModels = new HashMap<>();
	...
	public void onResourceManagerReload(IResourceManager reloadManager) {
		...
		for (Entry<ResourceLocation, ModelMHMD> entry : this.cachedModels
				.entrySet()) {
	 ...
	 */
	public boolean registerModel(ModelMHMD model) {
		if (model == null)
	...
	/**
	 * Loads a {@link ModelMHMD} from the {@link ResourceLocation} given. If a
	 ...
	 * @return a {@link ModelMHMD} that corresponds to the requested
	 *         {@link ResourceLocation}
	 */
	public ModelMHMD loadModelFrom(ResourceLocation resLocation) {
		ModelMHMD model = this.cachedModels.get(resLocation);
		if (model == null) {
			logger.trace(String.format(
					"[ModelRegistry] Loading new model from %s", resLocation));
			// Not registered
			model = MHMDModelLoader.instance.loadInstance(resLocation,
