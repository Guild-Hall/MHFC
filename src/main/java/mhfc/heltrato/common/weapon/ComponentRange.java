...
import mhfc.heltrato.common.entity.projectile.EntityProjectile;
import mhfc.heltrato.common.util.Attributes;
import mhfc.heltrato.common.weapon.range.ReloadHelper;
...
public abstract class ComponentRange extends AbstractWeaponClass
{
	...
	{
		multimap.put(Attributes.RELOAD_TIME.getAttributeUnlocalizedName(), new AttributeModifier(weapon.getUUID(), "Weapon reloadtime modifier", rangedSpecs.getReloadTime(), 0));
	}
	...
	//	FLINTLOCK("weaponmod:bullet", "flintlock", 80, 1);
		...
		public int getReloadTime()
		{
			...
			{
				reloadTime = MHFCMain.instance.modConfig.getReloadTime(reloadTimeTag);
				MHFCMain.modLog.debug("Found reaload time " + reloadTime + " for " + reloadTimeTag + " @" + this);
			}
			return reloadTime;
		}
		...
		public Item getAmmoItem()
		{
			if (ammoItem == null && ammoItemTag != null)
			{
				...
				MHFCMain.modLog.debug("Found item " + ammoItem + " for " + ammoItemTag + " @" + this);
				ammoItemTag = null;
			}
			return ammoItem;
		}
