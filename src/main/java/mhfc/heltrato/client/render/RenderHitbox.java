...
public class RenderHitbox {
	...
	//TODO
	/**
	 * renderVert is a method that will be use to get the right angles of the current entity
	 * hitboxes . Using the math flow the render will continuely adjust its edges and stick
	...
	public void renderVert(float getBoxX, float getBoxY, float getBoxZ, int width, int length) {
		for(width = 0; width < length; width ++){
			for(length = 0; length < ri4; length ++){
				...
				int hitZ = Math.floorDiv(length, width);
				if(length < width && length == idle - hitZ){
					...
					for(int obs = 0; obs < quarter; obs++){
						...
						double dE;
						double dF;
						Render.renderOffsetAABB(AxisAlignedBB.getBoundingBox(dA * obs, dB * obs - hitZ, dA * obs, dD / obs, dB * hitZ, dD / obs), 2, 2, 2);
					}
				}
			}
		}
	...
	private void getMetaID(String name, int id, boolean soft) {
		...
		RenderGlobal.drawOutlinedBoundingBox(AxisAlignedBB.getBoundingBox(rf1, rf2, rf3, rf4, ri1, ri2), traceEvery);
		saveStateBoxes(name);
		}
	}
