package mhfc.heltrato.client.render.weapon.bow;

import mhfc.heltrato.client.model.weapon.bow.ModelBTigrexArrow;
...
public class RenderBTigrexArrow implements IItemRenderer {

	private ModelBTigrexArrow weapon;
	// private RenderWeapon getRender;
	private float scale;
	public RenderBTigrexArrow() {
		weapon = new ModelBTigrexArrow();
	}

	@Override
	public void renderItem(ItemRenderType type, ItemStack item, Object... data) {

		int usingItem = ((WeaponBTigrexArrow) item.getItem()).usingItem;
		switch (type) {
				...
				if ((((Entity) data[1] instanceof EntityPlayer))
					...
					if (usingItem < 5) {
						Minecraft.getMinecraft().renderEngine
								.bindTexture(new ResourceLocation(MHFCReference.weapon_bow_tigrexarrow_tex0));
					}
					if ((usingItem >= 5) && (usingItem < 35)) {
						Minecraft.getMinecraft().renderEngine
								.bindTexture(new ResourceLocation(MHFCReference.weapon_bow_tigrexarrow_tex1));
					}
					if ((usingItem >= 35)) {
						Minecraft.getMinecraft().renderEngine
								.bindTexture(new ResourceLocation(MHFCReference.weapon_bow_tigrexarrow_tex2));
					} else {
						Minecraft.getMinecraft().renderEngine
								.bindTexture(new ResourceLocation(MHFCReference.weapon_bow_tigrexarrow_tex0));
					}
