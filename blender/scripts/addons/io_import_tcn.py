...
class Techne(Element):
	...
	def apply(self):
		...
		for model in self.models:
			...
			if obj is None:
				continue
			if hasattr(obj.data, 'mhfcprops'):
				props = obj.data.mhfcprops
...
class Model(Element):
	...
	def apply(self):
		...
		mhfc_loop = None
		if hasattr(mesh, 'mhfcprops'):
			mhfc_loop = bm.faces.layers.int.new('MHFCRenderGroupIndex')
		for shape_nbr, shape in enumerate(self.shapes):
			...
			for face in idx_uvs:
				...
				if mhfc_loop is not None:
					bface[mhfc_loop] = shape_nbr + 1
